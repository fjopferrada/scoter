...
** STARTED 042 Move and activate clipping limit text boxes
...
** TODO 045 Add "revert to initial configuration" to GUI
   ...
   re-read.
** TODO 046 Keep clip limits when clearing data
   Currently, when a series is cleared, the clipping limits are removed. This
   should only happen if there is no data for either parameter in the cleared
   role. For example, if the user clears the $\delta^{18}\mathrm{O}$ record
