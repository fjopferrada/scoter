...
def _find_executable_noext(leafname):
    ...
    def is_exe(path_to_check):
        return os.path.isfile(path_to_check) and \
               os.access(path_to_check, os.X_OK)
...
class Scoter(object):
    ...
    preprocessing them, and running the actual correlation. It provides a
    simple API suitable for use from the command line, from the companion GUI
    ScoterGui, or from other Python code importing the scoter module.
    """
    ...
    def has_series(self, role, parameter):
        return self.series[role][parameter] is not None

    def clear_data(self, role, parameter):
        """Clear a data series.
        
        Remove a previously read data series (record or target curve) from
        Scoter.
    ...
    def preprocess(self, config):
        ...
        assert((self.series[0][0] is not None and
                self.series[1][0] is not None) or
               (self.series[0][1] is not None and
                self.series[1][1] is not None))
        ...
        for parameter in (0, 1):
            ...
            if self.series[0][parameter] is not None and \
                    self.series[1][parameter] is not None:
        ...
        if config.interp_active:
            if config.interp_npoints == -2:  # use minimum
                ...
                raise Exception("Illegal interp_npoints value: %d" %
                                interp_npoints)
        ...
        record_end = \
            min([s.end() for s in series_picked[0]] +
                ([config.record_end] if config.record_end > -1 else []))
        target_end = \
            min([s.end() for s in series_picked[1]] +
                ([config.target_end] if config.target_end > -1 else []))

        logger.debug(str(record_end))

        series_truncated = [map(lambda s: s.clip((record_start, record_end)),
                                series_picked[0]),
                            map(lambda s: s.clip((target_start, target_end)),
                                series_picked[1])]
        ...
        def preproc(series):
            ...
            if config.normalize:
                if series.parameter == "d18o":
                    ...
                    raise Exception("Unknown parameter type: %s" %
                                    series.parameter)
            return result
        ...
        # Rename series to reflect their role in the correlation. Apart from
        # anything else, this ensures that there won't be any name clashes if
        # they are written to files for the use of the Match program.
    ...
    def correlate_sa(self, known_line, config, callback_obj):
        ...
        starting_warp = Bwarp(Bseries(self.series_preprocessed[0],
                                      config.sa_intervals),
                              Bseries(self.series_preprocessed[1],
                                      config.sa_intervals),
        ...
        if config.live_display:
            plotter = WarpPlotter(config.sa_intervals, known_line, 100,
                                  pdf_file='dsaframes-1.pdf' if
                                  config.make_pdf else None)
        ...
        def callback(soln_current, soln_new, schedule):
            ...
            if callback_obj is not None:
                pc = (ltemp_max - math.log(schedule.temp)) /\
                     (ltemp_max - ltemp_min) * 100
        ...
        schedule = AdaptiveSchedule(config.temp_init, config.temp_final,
                                    config.max_changes, config.max_steps,
                                    rate=config.cooling)
    ...
    def correlate_match(self, config, remove_files=False):
        ...
        match_conf = MatchConf(
            MatchSeriesConf(self.series_preprocessed[0],
                            intervals=config.match_intervals),
            MatchSeriesConf(self.series_preprocessed[1],
                            intervals=config.match_intervals),
            match_params)
        match_path = self.default_match_path if config.match_path == ""\
            else config.match_path
        logger.debug("Match path: %s", match_path)
        if match_path is None or match_path == "":
    ...
    def save_results(self, directory = None):
        ...
        if self.aligned_sa:
            for i in range(len(self.aligned_sa)):
                self.aligned_sa[i].write(
                    os.path.join(path, "data-simann-%d" % i))
    ...
    def finalize(self):
        ...
        # It's important to remove the handler, in case perform_complete
        # correlation is being run from the GUI: in this case, the GUI will
        # keep using the logger afterwards, but there's no guarantee that the
        # file will remain accessible.
        self.file_log_handler.close()
        logger.removeHandler(self.file_log_handler)

    def add_file_log_handler(self):
        self.file_log_handler =\
            logging.FileHandler(os.path.join(self.output_dir, "scoter.log"))
    ...
    def perform_complete_correlation(self, config_file):
        ...
        if config.scoter_version != SCOTER_VERSION:
            logger.warn("Configuration has version %s, "
                        "but Scoter has version %s" %
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Correlate geological records.")
    parser.add_argument("configuration", metavar="filename", type=str,
                        nargs="?",
    ...
    parser.add_argument("--write-config", metavar="filename", type=str,
                        help="write a configuration template to the supplied "
                             "filename")
    parser.add_argument("--overwrite", action="store_true",
                        help="when writing a configuration template, "
                             "overwrite any existing file")
    parser.add_argument("--log-level", metavar="level", type=str,
                        help="logging level (non-negative integer or "
    ...
    stderr_handler.setFormatter(
        logging.Formatter("%(levelname)-8s: %(message)s", None))
    ...
    if args.write_config:
        ...
        if os.path.isfile(args.write_config):
            ...
            logger.error("'%s' is a directory; not writing configuration." %
                         args.write_config)
        else:
            ok_to_write = True
